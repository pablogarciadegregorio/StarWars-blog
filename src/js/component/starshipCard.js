...
export const StarshipCard = props => {
  ...
  return (
    <div className="card ms-0 me-2 mt-1" style={{ width: "18rem" }}>
      ...
      <div className="card-body ms-2 me-2">
        ...
        <p className="card-text">Cost: 100.000.000.000 credits</p>
        <p className="card-text">Passangers : 950.320</p>

        <div className=" container d-flex p-0">
          <button href="#" className="btn btn-primary m-0 " url={props.url} onClick={e => navigate("/singleStarship/" + props.uid)}>Learn More</button>
          <button className="btn btn-warning like ms-auto " onClick={e => handleFavorite(e)}><i className={store.favorites.indexOf(props.name) == -1 ? "like fa-regular fa-heart" : "like fa-solid fa-heart"}></i></button>
        </div>
      </div>
    </div>
  );
